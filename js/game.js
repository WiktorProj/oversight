...
var interval = setInterval(function() {
	...
	if (inChallenge("s",11) && hasMilestone("r",1)) {
		let timeleft = new Decimal(500).sub(player.points).div(getPointGen())
		displayThings = [
			`Challenge ~↓ : will finish challenge in ${timeleft.gt(0)?formatTime(timeleft):"now"}`
		]
	}
	if (inChallenge("s",21) && hasMilestone("r",1)) {
		let timeleft = new Decimal(200).sub(player["specifics"]).div(player["specificsspeed"].times(player["specificsgain"]))
		displayThings = [
			`Challenge ~↓↓↓ : will finish challenge in ${player["depositingrespecs"]?(timeleft.gt(0)?formatTime(timeleft):"now"):"never"}`,
			`Challenge ~↓↓↓ : you need ${format(new Decimal(200).sub(player["specifics"]).div(player["specificsgain"]))} respecs to finish`
		]
	}
	...
	if (player["depositingrespecs"]) {
		...
		player["specifics"] = player["specifics"].add(deposit.times(player["specificsgain"]))
		if (player["r"].points.eq(0)) {
			player["depositingrespecs"] = false
		}
	}
